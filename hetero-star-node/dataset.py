...
import numpy as np
from utils import get_ids, load_bert_feats, load_data, load_clip_feats
import pandas as pd
...
class WebQnaDataset(Dataset):
    ...
    def get(self, idx:int)-> Data:
        ...
        if len(y_img) == 0:
            ...
            # img_feat = torch.zeros(512+768) #clip base
            img_feat = torch.zeros(768+768) # clip Vi-L
            # img_feat = torch.zeros(1024+768)
        ...
        data['img_src'].num_srcs= num_img_srcs
        data['txt_src'].num_srcs= num_txt_srcs
        data['ques'].num_txt_src= num_txt_srcs
        data['ques'].num_img_src= num_img_srcs
        data['ques'].category = self._WebQna_dataset[id]["Qcate"]
        data['ques'].id= id
